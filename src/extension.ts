...
export function activate(context: vscode.ExtensionContext) {
    context.subscriptions.push(
        vscode.commands.registerCommand('devcmd.setEnvironment', async () => {
            try {
                await setEnvironment(context);
                vscode.window.showInformationMessage(`${context.environmentVariableCollection.description} successfully set.`)
            } catch (error: any) {
...
async function setEnvironment(context: vscode.ExtensionContext) {
    ...
    const vars = await vscode.window.withProgress({
        ...
        return vcvars.getVCVars(vs, options);
    });
